...
export default function ProjectItem({
    ...
    // Check if this is the specific URL we want to apply the animation to
    const isSpecialPage = pathname === "/takmeel-al-barari-view-properties" || url === "takmeel-al-barari-view-properties"

    // Store the initial scroll position and lock scroll only for the special page
    useEffect(() => {
        if (isSpecialPage) {
    ...
    // Transform the scroll progress to SVG scale and opacity
    const svgScale = useTransform(scrollProgress, [0, 1], [0.5, 1])
    const svgOpacity = useTransform(scrollProgress, [0, 0.3, 1], [0, 0.6, 1])

    // Monitor scroll progress to unlock scrolling when animation completes (only for special page)
    ...
    return (
        <div className="FullScreenBanner Projects" ref={containerRef}>
            ...
            {/* SVG Animation - conditionally controlled by scroll for special page */}
            {isSpecialPage && (
                <motion.div
                    className="LottieBox"
                    style={
            ...
            {/* Scroll indicator - only shown for special page when scrolling is locked */}
            {isSpecialPage && scrollLocked && (
                <motion.div
